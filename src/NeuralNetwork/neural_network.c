                                             ...
                                             int arr[size_of_arr])
{
    ...
    link_layer_input(neural_network->layers[0], neural_network->input_size,
 ...
 */
long double
categorical_cross_entropy(const struct neural_network *neural_network,
...
void minimise_loss(const struct neural_network *neural_network,
                   char expected_output, long double shift)
{
    ...
    for (int i = 0; i < neural_network->number_of_layers; ++i)
    {
        ...
        for (int j = 0; j < layer->layer_size; ++j)
        {
            ...
            for (int k = 0; k < layer->prev_layer_size; ++k)
            {
                ...
                else if (!(minus_shift_loss < current_loss &&
                           minus_shift_loss < plus_shift_loss))
                {
                    layer->weights[j][k] = original_weight;
                }

                if (isnanf(categorical_cross_entropy(neural_network,
                                                     expected_output)))
                {
            ...
            if (current_loss < minus_shift_loss &&
                current_loss < plus_shift_loss)
            {
                layer->biases[j] = original_bias;
            }
            else if (plus_shift_loss < current_loss &&
                     plus_shift_loss < minus_shift_loss)
            {
                layer->biases[j] += shift * 2;
            }
